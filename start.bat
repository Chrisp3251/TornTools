...
setlocal
cd /d "%~dp0"
title TornTools v0.3.7 Backend

echo TornTools v0.3.7 Backend
echo Dashboard: http://127.0.0.1:8765
echo Mug Scout: http://127.0.0.1:8765/static/mug-scout.html
echo.
echo Clearing any stale TornTools listener on port 8765...
powershell.exe -NoProfile -Command "$p=(Get-NetTCPConnection -LocalPort 8765 -State Listen -ErrorAction SilentlyContinue).OwningProcess ^| Sort-Object -Unique; if($p){$p ^| ForEach-Object { Stop-Process -Id $_ -Force -ErrorAction SilentlyContinue }; Start-Sleep -Seconds 1}"
echo.
