...
function configuredSniperMax(target) {
  return Number((target && target.configured_max_price) || (target && target.max_price) || 0);
}

function effectiveSniperMax(target) {
...
function nextSniperTarget() {
  const now = Date.now();
  const enabled = Array.from(sniperTargets.values()).filter(x => x.enabled);
  let best = null;
...
function processSniperResult(target) {
  ...
  const spread = max - Number(x.lowest || 0);
  notify(
    `SNIPER · ${target.name}`,
    `${money.format(x.lowest)} is at/below your live-safe ${money.format(max)} max${spread > 0 ? ` · ${money.format(spread)} under max` : ""}. Click to open the market.`,
...
function renderSniperTargets() {
  ...
  if (!rows) return;
  const targets = Array.from(sniperTargets.values()).sort((a,b) => Number(b.enabled)-Number(a.enabled) || String(a.name).localeCompare(String(b.name)));
...
function ensureSniperCandidatePanel() {
  ...
  const sniperRows = $("sniperRows");
  const sniperSection = sniperRows ? sniperRows.closest("section") : null;
  if (!sniperSection) return;
...
function renderSniperCandidates(items, requirements) {
  ...
  const filtered = Array.from(items || []).slice(0, 16);
  if (!filtered.length) {
...
async function saveSniperTarget() {
  const id = Number($("sniperItemId") && $("sniperItemId").value || 0);
  const name = String($("sniperName") && $("sniperName").value || "").trim();
  const maxPrice = Math.trunc(Number($("sniperMaxPrice") && $("sniperMaxPrice").value || 0));
  if (!id || maxPrice <= 0) return msg("Sniper needs a valid item ID and max buy price.", true);
  const existing = sniperTargets.get(id);
  try {
    const d = await call("/api/sniper/watchlist", {
      method:"POST",
      body:JSON.stringify({item_id:id, name:name || (existing && existing.name) || `Item ${id}`, max_price:maxPrice, enabled: existing ? existing.enabled : true})
    });
    if (Array.isArray(d.discovery_ids)) discoveryIds = d.discovery_ids.map(Number);
    $("sniperItemId").value = "";
    $("sniperName").value = "";
    $("sniperMaxPrice").value = "";
    await loadSniperTargets();
    await loadSniperCandidates();
    if (!sniperRunning) startSniperWatch();
  } catch (e) { msg(e.message, true); }
...
function editSniperTarget(id) {
  ...
  $("sniperMaxPrice").value = configuredSniperMax(t);
...
async function toggleSniperTarget(id) {
  ...
  try {
    const d = await call("/api/sniper/watchlist", {
      method:"POST",
      body:JSON.stringify({item_id:t.item_id, name:t.name, max_price:configuredSniperMax(t), enabled:!t.enabled})
    });
    if (Array.isArray(d.discovery_ids)) discoveryIds = d.discovery_ids.map(Number);
    await loadSniperTargets();
    await loadSniperCandidates();
...
function updateSniperStatus(next) {
  ...
  const enabled = Array.from(sniperTargets.values()).filter(x => x.enabled);
  if (!enabled.length) {
    nextEl.textContent = "No enabled sniper targets.";
    return;
  }
...
function stopSniperWatch() {
  ...
  const btn = $("sniperToggleBtn");
  if (btn) {
    btn.textContent = "Start Sniper Watch";
...
function toggleSniperWatch() {
  if (sniperRunning) stopSniperWatch();
  else startSniperWatch();
}
