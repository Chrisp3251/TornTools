...
async def fetch_market(client,i,limit=100):
    ...
    try:
        r=await client.get(f"{API_BASE}/market/{i}/itemmarket",headers={"Authorization":f"ApiKey {_api_key}"},params={"limit":limit,"offset":0}); d=r.json()
    except httpx.RequestError as e: raise HTTPException(502,f"Could not reach Torn API: {e}") from e
    except ValueError as e: raise HTTPException(502,"Torn API returned unreadable data") from e
    err=torn_error(d)
